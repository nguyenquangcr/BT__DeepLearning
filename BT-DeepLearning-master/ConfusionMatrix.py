...
confusion_matrix = tf.contrib.metrics.confusion_matrix(test_labels,predicted)

with tf.Session() as sess:
    import numpy as np
    cm = sess.run(confusion_matrix)
    cm_normalize = cm/cm.astype(np.float).sum(axis=1)[:, np.newaxis]
    np.savetxt("confusion_matrix.csv", cm, delimiter="\t",fmt="%d")
    np.savetxt("normalize_confusion_matrix.csv", cm_normalize, delimiter="\t",fmt="%5.3f")
